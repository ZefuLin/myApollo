...
void mapping(double pose[7], double pointCloud[3000][6],
    ...
    *source_cloud =*myTransformPointCloud(source_cloud,scr2global);
    ...
    seq_src++;
    // 展示global点云
    auto final_global_cloud = std::make_shared<PointCloud>();
    int final_global_cloud_size = global_cloud->size();
    apollo::cyber::Init("mapping.cc");
    auto talker_node = apollo::cyber::CreateNode("global_cloud_publisher");
    auto talker_pointCloud = talker_node->CreateWriter<PointCloud>("channel/publishGlobalCloud");
    apollo::cyber::Rate rate(1.0);
    for (int i=0; i<final_global_cloud_size; i++){
        const auto &point = global_cloud->points[i];
        final_global_cloud->add_point();
        final_global_cloud->mutable_point(i)->set_x(point.x);
        final_global_cloud->mutable_point(i)->set_y(point.y);
        final_global_cloud->mutable_point(i)->set_z(point.z);
        int r,g,b,intensity;
        r = point.r;
        g = point.g;
        b = point.b;
        intensity = ((r*256+g)*256+b);
        final_global_cloud->mutable_point(i)->set_intensity(intensity);
    }
    talker_pointCloud->Write(final_global_cloud);
    rate.Sleep();
